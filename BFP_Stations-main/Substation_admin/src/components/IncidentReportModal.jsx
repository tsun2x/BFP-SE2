...
function formatCoordinates(alarm) {
  const lat = Number(alarm?.user_latitude);
  const lng = Number(alarm?.user_longitude);
...
export default function IncidentReportModal({ alarmId, onClose }) {
  ...
  useEffect(() => {
    ...
    apiCall(`/incidents/${alarmId}/report-data`)
      .then((data) => {
        setReportData(data);
        const initialTimeline = parseTimelineDetails(data?.timeline?.[0]?.details);
        const coordinatesText = formatCoordinates(data?.alarm);
        const fallbackLocation =
          data?.report?.location ||
          initialTimeline.location ||
          (coordinatesText !== 'N/A' ? coordinatesText : '');
        const fallbackNarrative = data?.report?.narrative || initialTimeline.narrative || '';
        ...
        if (data.report) {
          ...
          setForm((f) => ({
            ...f,
            incident_type: data.alarm.incident_type || initialTimeline.incidentType || '',
            location: fallbackLocation,
            narrative: fallbackNarrative,
          }));
        }
          ...
          <>
            ...
            <div style={styles.summaryBox}>
              ...
              <div style={styles.summaryRow}>
                ...
                <span>{formatCoordinates(reportData.alarm)}</span>
              </div>
              <div style={styles.summaryRow}>
                <span style={styles.summaryLabel}>Reported Location</span>
                <span>{form.location || reportData.report?.location || 'N/A'}</span>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} style={styles.form}>
              ...
              <label style={styles.label}>Reported Location</label>
              <input
                style={styles.input}
                name="location"
                value={form.location}
                onChange={handleChange}
                placeholder="Street, landmark, or lat/lng"
              />

...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res.status(401).json({ message: 'No account found. Please check your phone number or ID and try again.' });
    }
...
router.post('/send-otp', async (req, res) => {
  try {
    ...
    if (phone) {
      ...
      return res.json({ success: true, method: 'phone', message: 'OTP sent via SMS' });
    } else {
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (phone) {
      ...
      if (error) {
        console.error('[POST /verify-otp] Supabase email verify error:', error.message);
        return res.status(400).json({ message: error.message || 'Invalid code. Please try again.' });
      }
...
router.post('/end-user-signup', async (req, res) => {
  try {
    ...
    console.log('[POST /end-user-signup] Registered civilian:', normPhone || normEmail);

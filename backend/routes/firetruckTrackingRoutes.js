...
router.put('/firetrucks/status', authenticateToken, requireRoles(['admin', 'substation_admin', 'driver']), async (req, res) => {
  try {
    ...
    // NOTE: Socket broadcast removed here to avoid double-notification.
    // The mobile app already emits 'truck-status-update' via socket directly,
    // so the REST endpoint only persists to the database.
    console.log('[firetrucks/status] Saved status for truck', truck_id, '(socket broadcast handled by client)');

...
async function deleteOtpRecord(otpId) {
  await supabase.from("otp_verifications").delete().eq("otp_id", otpId);
}

async function removeActiveOtpRecords(target, channel) {
...
async function createOtpRecord({
  ...
  await removeActiveOtpRecords(target, channel);
  ...
  const { error } = await supabase.from("otp_verifications").insert([payload]);
...
async function findActiveOtpRecord(target, channel) {
  const now = new Date().toISOString();
  const { data, error } = await supabase
    .from("otp_verifications")
    .select("otp_id, code_hash, expires_at, provider")
    .eq("target", target)
    .eq("channel", channel)
    .eq("purpose", OTP_PURPOSE)
    .is("consumed_at", null)
  ...
  if (new Date(data.expires_at).toISOString() <= now) {
    await deleteOtpRecord(data.otp_id);
    return { expired: true };
  }
...
async function consumeOtpRecord(otpId) {
  const { error } = await supabase
    .from("otp_verifications")
...
function createEmailTransport() {
  ...
  const brevoFromName = String(
    process.env.BREVO_FROM_NAME || "Proteksyon BFP",
  ).trim();
  ...
  const brevoHost = String(process.env.BREVO_SMTP_HOST || "").trim();
  const brevoPort = Number(process.env.BREVO_SMTP_PORT || 587);
  const brevoUser = String(process.env.BREVO_SMTP_USER || "").trim();
  const brevoPassword = String(process.env.BREVO_SMTP_PASSWORD || "").trim();

  if (brevoHost && brevoUser && brevoPassword && brevoFromEmail) {
    ...
    try {
      ...
      if (error?.code === "EAUTH" || Number(error?.responseCode) === 535) {
        return res.status(500).json({
          message:
            "Email login rejected by Gmail. Use a Google App Password (16 chars) for GMAIL_APP_PASSWORD or GMAIL_PASS. If you copied it with spaces, keep it as-is; backend now normalizes spaces automatically.",
        });
      }
...
router.put("/me", authenticateToken, async (req, res) => {
  try {
    ...
    if (securitySettings && typeof securitySettings === "object")
      updates.security_settings = securitySettings;
...
// ── Send OTP (phone via Twilio SMS, email via SMTP) ──────────────
  ...
  try {
    ...
    if (phone) {
      ...
      try {
        ...
        await removeActiveOtpRecords(to, "phone");
        throw smsError;
      }
      ...
      // Email: use Brevo SMTP when configured, otherwise fall back to Gmail SMTP.
      ...
      if (!mailer) {
        return res.status(500).json({
          message:
            "Email provider is not configured. Set BREVO_SMTP_* or GMAIL_* environment variables.",
        });
      }
      ...
      try {
        await mailer.sendMail({
          ...
          text: `Your Proteksyon verification code is: ${code}\n\nThis code expires in 5 minutes.`,
        ...
        await removeActiveOtpRecords(normEmail, "email");
        console.error(
          `[POST /send-otp] ${mailer.provider} email error:`,
          err?.response?.data || err?.message || err,
        );
        return res.status(500).json({
          message: `Failed to send OTP email via ${mailer.provider}`,
          error: err?.response?.data?.message || err?.message || String(err),
        });
      }
      console.log(
        `[POST /send-otp] ${mailer.provider} email OTP sent to`,
        normEmail,
      );
      return res.json({
        success: true,
...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    if (phone) {
      const key = normalizePhoneForOtp(phone);
      const stored = await findActiveOtpRecord(key, "phone");
      ...
      if (stored.code_hash !== hashOtp(otp)) {
        return res
          .status(400)
          .json({ message: "Invalid code. Please try again." });
      }

      await consumeOtpRecord(stored.otp_id);
      console.log("[POST /verify-otp] Phone verified:", key);
      return res.json({ success: true, message: "Verified successfully" });
    } else {
      // Email: verify the DB-backed OTP created during send-otp.
      const normEmail = normalizeEmailForOtp(email);

      const stored = await findActiveOtpRecord(normEmail, "email");
      if (!stored) {
        return res
          .status(400)
          .json({ message: "No OTP found. Please request a new code." });
      ...
      await consumeOtpRecord(stored.otp_id);
      console.log(
        "[POST /verify-otp] Email verified via stored OTP:",
        normEmail,
      );
      return res.json({ success: true, message: "Verified successfully" });
    }
...
// Body: { phone?, email?, otp, newPassword }
router.post("/reset-password", async (req, res) => {
  try {
    ...
    if ((!phone && !email) || !otp || !newPassword) {
      return res
        .status(400)
        .json({ message: "Phone/email, OTP, and new password are required" });
    }
    ...
    let target, channel;
    if (phone) {
      target = normalizePhoneForOtp(phone);
      channel = "phone";
    } else {
      target = normalizeEmailForOtp(email);
      channel = "email";
    }

    const stored = await findActiveOtpRecord(target, channel);
    if (!stored) {
      return res
        .status(400)
        .json({ message: "No OTP found. Please request a new code." });
    ...
    // 2. Find the user by phone or email
    let userQuery;
    if (phone) {
      const variants = [target];
      if (target.startsWith("+63")) variants.push("0" + target.slice(3));
      const { data, error } = await supabase
        .from("users")
        .select("user_id")
    ...
    if (userQuery.error) {
      console.error(
        "[POST /reset-password] user lookup error:",
        userQuery.error,
      );
      return res
        .status(500)
    ...
    if (!userQuery.data) {
      return res
        .status(404)
        .json({ message: "No account found with that phone/email" });
    }
    ...
    if (updateError) {
      console.error("[POST /reset-password] update error:", updateError);
      return res
        .status(500)
        .json({
          message: "Failed to reset password",
    ...
    console.log(
      "[POST /reset-password] Password reset for user:",
      userQuery.data.user_id,
    );
    return res.json({
      success: true,
    ...
    console.error("[POST /reset-password] Exception:", e);
    return res
      .status(500)
      .json({ message: "Failed to reset password", error: e.message });
  }
});
...
router.post("/signup", async (req, res) => {
  try {
    ...
    const requestedStationType = String(
      req.body.stationType || "",
    ).toLowerCase();

    if (["admin", "substation_admin", "driver"].includes(requestedRole)) {
      role = requestedRole;
    } else if (requestedStationType === "main") {
      role = "admin";
    } else if (requestedStationType === "substation") {
    ...
    if (assignedStationId) {
      ...
      // Enforce: substation admin accounts cannot be assigned to Main
      if (roleLower === "substation_admin" && stationType === "main") {
        return res.status(400).json({
          message:
            "Substation admin users cannot be assigned to the Main station",
        });
      }
    ...
    // --- PERMANENT FIX: If role is driver, insert into firetrucks table ---
    if (role === "driver" && newUser && newUser[0]) {
      const driverId = newUser[0].user_id;
      const truckInsert = {
        truck_code: `TRUCK_${idNumber}`,
        truck_name: `${fullName}'s Truck`,
        assigned_station_id: assignedStationId,
        truck_type: req.body.truckType || "Standard",
      ...
      if (truckError) {
        console.error(
          "Failed to create firetruck for driver:",
          truckError.message,
        );
        // Optionally: return a warning to the client, but still allow user creation
        return res.status(201).json({
          message: "User registered, but failed to create firetruck.",
          warning: truckError.message,
        });
      }
    }
...
router.get("/me", authenticateToken, async (req, res) => {
  try {
    ...
    const userObj = {
      ...
      securitySettings: user.security_settings || null,
    };

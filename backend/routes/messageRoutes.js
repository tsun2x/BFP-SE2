...
const MESSAGE_ATTACHMENTS_BUCKET =
  process.env.MESSAGE_ATTACHMENTS_BUCKET || "message-attachments";
...
const withAttachmentDownloadUrls = async (messages) => {
  ...
  for (const m of rows) {
    const atts = Array.isArray(m.message_attachments)
    ...
    for (const att of atts) {
      ...
      try {
        ...
        if (error) {
          ...
          mappedAtts.push({
            ...att,
            download_url: data?.signedUrl || att.file_url || null,
          });
        }
      } catch {
        mappedAtts.push({ ...att, download_url: att.file_url || null });
...
const getConversationById = async (conversationId) => {
  const { data, error } = await supabase
    .from("conversations")
    .select(
      "conversation_id, station_a_id, station_b_id, created_at, updated_at",
    )
...
const ensureConversationAccess = (req, conversation, senderStationId) => {
  ...
  if (!allowed) {
    const err = new Error(
      "Forbidden: conversation is not accessible from your station",
    );
    err.statusCode = 403;
    throw err;
  }
  ...
  async (req, res) => {
    try {
      ...
      let query = supabase
        .from("conversations")
        .select(
          "conversation_id, station_a_id, station_b_id, created_at, updated_at, archived_by_a, archived_by_b",
        )
      ...
      if (!isAdmin) {
        ...
        query = query.or(
          `station_a_id.eq.${stationId},station_b_id.eq.${stationId}`,
        );
      }
      ...
      const stationIds = Array.from(
        new Set(
          rows
            .flatMap((c) => [c.station_a_id, c.station_b_id])
            .filter(Boolean)
            .map(String),
        ),
      );
      ...
      if (stationIds.length) {
        ...
        if (stationsErr) {
          ...
          stationsById = Object.fromEntries(
            (stations || []).map((s) => [String(s.station_id), s]),
          );
        }
      }
      ...
      for (const c of rows) {
        ...
        try {
          const { data: lastRows, error: lastErr } = await supabase
            .from("messages")
            .select(
              "message_id, subject, body, sent_at, sender_station_id, recipient_station_id, is_read",
            )
        ...
        // Filter by archive status based on ?archived= query param
        if (!isAdmin && stationId) {
          const isSideA = String(c.station_a_id) === String(stationId);
          const isArchivedForMe = isSideA ? c.archived_by_a : c.archived_by_b;
          if (showArchived && !isArchivedForMe) continue;
          if (!showArchived && isArchivedForMe) continue;
        } else if (isAdmin) {
          const isArchivedForAdmin = c.archived_by_a && c.archived_by_b;
          if (showArchived && !isArchivedForAdmin) continue;
          if (!showArchived && isArchivedForAdmin) continue;
        }
        ...
        result.push({
          ...
          otherStationId,
          otherStationName: otherStationId
            ? stationsById[String(otherStationId)]?.station_name || null
            : null,
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch conversations", error: e.message });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!recipientStationId) {
        return res
          .status(400)
          .json({ message: "recipientStationId is required" });
      }
      if (String(recipientStationId) === String(senderStationId)) {
        return res
          .status(400)
          .json({ message: "Cannot message your own station" });
      }
      if (!body) {
      ...
      if (existingErr) {
        console.error(
          "[POST /conversations] find existing error:",
          existingErr,
        );
        return res.status(500).json({
      ...
      if (msgErr) {
        ...
        return res
          .status(500)
          .json({ message: "Failed to send message", error: msgErr.message });
      }
      ...
      return res
        .status(500)
        .json({ message: "Failed to create conversation", error: e.message });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      const { data: messages, error } = await supabase
        .from("messages")
        .select(
          "*, message_attachments(attachment_id, file_url, file_name, mime_type, file_size_bytes)",
        )
        .eq("conversation_id", conversationId)
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (msgErr) {
        console.error(
          "[POST /conversations/:id/messages] insert error:",
          msgErr,
        );
        return res
          .status(500)
      ...
      return res
        .status(status)
        .json({ message: "Failed to send message", error: e.message });
    }
  },
);

// Mark all messages in a conversation as read for the current station
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      // For non-admin users with a known station, only mark messages where they are the recipient.
      // For admin users with no assigned station, mark all unread messages in the conversation
      // (they represent the main station and can see all messages).
      if (stationId) {
        query = query.eq("recipient_station_id", stationId);
      } else if (!isAdmin) {
        return res.status(403).json({ message: "Not assigned to any station" });
      }

      const { data: updated, error } = await query.select("message_id");
      ...
      console.error("[PATCH /conversations/:id/read] exception:", e);
      return res
        .status(500)
        .json({ message: "Failed to mark as read", error: e.message });
    }
  },
);

// Archive a conversation for the requesting user's side
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      const isAdmin = isAdminUser(req.user);
      const conversation = await getConversationById(conversationId);
      ensureConversationAccess(req, conversation, stationId);

      let updateCol;
      if (isAdmin && !stationId) {
        // Admin with no station: archive both sides
        updateCol = { archived_by_a: true, archived_by_b: true };
      } else {
        const isSideA = String(conversation.station_a_id) === String(stationId);
        updateCol = isSideA ? { archived_by_a: true } : { archived_by_b: true };
      }
      ...
      if (error) {
        console.error("[PATCH /conversations/:id/archive] error:", error);
        return res
          .status(500)
          .json({ message: "Failed to archive", error: error.message });
      }

      return res.json({ success: true });
    } catch (e) {
      const status = e.statusCode || 500;
      console.error("[PATCH /conversations/:id/archive] exception:", e);
      return res
        .status(status)
        .json({ message: "Failed to archive", error: e.message });
    }
  },
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      return res
        .status(status)
        .json({ message: "Failed to delete conversation", error: e.message });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      for (const file of req.files) {
        ...
        if (uploadErr) {
          console.error(
            "[POST /messages/:id/attachments] storage error:",
            uploadErr,
          );
        ...
        const { data: row, error: insertErr } = await supabase
          ...
          .select(
            "attachment_id, file_url, file_name, mime_type, file_size_bytes",
          )
          .single();
      ...
      return res
        .status(500)
        .json({ message: "Failed to upload attachments", error: e.message });
    }
  },
);

...
export const NewsRoomScreen: React.FC = () => {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: palette.pageBg }]} edges={['top']}>
      <ScrollView
        style={[styles.scrollView, { backgroundColor: palette.pageBg }]}
        contentContainerStyle={[styles.scrollContent, { backgroundColor: palette.pageBg }]}
      >
        <View style={styles.banner}>
          <Text style={[styles.bannerTitle, { fontSize: 20 * fontScale }]}>Welcome to the News Room</Text>
          <Text style={[styles.bannerSubtitle, { fontSize: 12 * fontScale }]}>Stay updated on incidents, and BFP announcements.</Text>
        </View>

        <View style={styles.sectionHeaderRow}>
          <Text style={[styles.sectionTitle, { color: palette.textPrimary, fontSize: 16 * fontScale }]}>Latest News</Text>
          <View style={styles.searchRow}>
            <View style={[styles.searchBox, { backgroundColor: palette.cardBg }]}> 
              <Ionicons name="search" size={16} color="#999" style={styles.searchIcon} />
              <TextInput
                style={[styles.searchInput, { color: palette.textPrimary, fontSize: 13 * fontScale }]}
                placeholder="Search"
        ...
        {loading ? (
          ...
          <Text style={{ textAlign: 'center', marginTop: 40, color: palette.textSecondary, fontSize: 14 * fontScale }}>No news articles found.</Text>
        ) : (
          <View style={styles.listWrapper}>
            {filtered.map((article) => (
              ...
              >
                {(article.heading_image || article.headline_image) ? (
                  <Image source={{ uri: article.heading_image || article.headline_image }} style={styles.cardImage} resizeMode="cover" />
                ) : (
                ...
                <View style={styles.cardOverlay}>
                  <Text style={[styles.cardTitle, { fontSize: 13 * fontScale }]} numberOfLines={2}>{article.title}</Text>
                  <Text style={[styles.cardDate, { fontSize: 11 * fontScale }]}>

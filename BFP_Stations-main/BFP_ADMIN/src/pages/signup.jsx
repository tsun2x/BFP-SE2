...
export default function Signup() {
  ...
  const validateForm = () => {
    ...
    if (!formData.idNumber.trim()) {
      ...
      newErrors.idNumber =
        "Invalid format. Use BFP- followed by 5 digits (e.g., BFP-01234)";
    }
  ...
  const handleSendOtp = async () => {
    if (otpCooldownSeconds > 0) {
      setOtpMessage(
        `Please wait ${otpCooldownSeconds}s before requesting a new OTP.`,
      );
      return;
    }
    ...
    try {
      ...
      setOtpMessage(
        msg.includes("<") ? "Failed to send OTP. Please try again." : msg,
      );
    } finally {
      setOtpLoading(false);
    }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      setOtpMessage(
        msg.includes("<") ? "Verification failed. Please try again." : msg,
      );
    } finally {
      setOtpLoading(false);
    }
          ...
          <form onSubmit={handleSignup}>
            ...
            <div className="auth-row two-columns">
              ...
              <div className="auth-group">
                ...
                >
                  ...
                  {!emailVerified && (
                    <button
                      ...
                      disabled={
                        otpLoading || !formData.email || otpCooldownSeconds > 0
                      }
                      className="otp-btn"
                    >
                      {otpLoading
                ...
                {otpMessage && (
                  <span
                    className={emailVerified ? "otp-success" : "otp-error"}
                    style={{
                      fontSize: "13px",
                      marginTop: "6px",
                      display: "block",

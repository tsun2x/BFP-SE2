...
// ...existing code...
  ...
  const deleteNews = (newsId) => {
    ...
    showConfirmModal(
      'delete',
      'Delete News Article',
      `Are you sure you want to delete "${newsItem?.title || 'this news article'}"? This action cannot be undone.`,
      async () => {
        setIsLoading(true)
        try {
          ...
          showNotification('error', 'Failed to delete news article.')
        }
        setIsLoading(false)
      }
    )
            ...
            <div className="nr-grid">
              {getFilteredNewsItems().map((n) => (
                <div key={n.id} className="nr-card" onClick={() => showNewsContent(n)}>
                  ...
                  <button 
                    className="nr-card-delete" 
                    title="Delete" 
                    onClick={(e) => { e.stopPropagation(); deleteNews(n.id); }}
                    style={{
                      position: 'absolute',
                      top: '8px',
                      right: '8px',

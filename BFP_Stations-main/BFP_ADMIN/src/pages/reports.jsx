    ...
    async function loadOfficers() {
      ...
      try {
        ...
        const res = await apiClient.get(
          `/officers?station_id=${recipientStationId}`,
        );
        // Accept multiple response shapes
        const potential =
          res?.officers || res?.users || res?.data || res?.rows || res || [];
        ...
        setOfficers([]);
        setOfficersError(
          e && e.message ? String(e.message) : "Failed to load officers",
        );
      } finally {
  ...
  return (
    <div className="composer-form">
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
        {officersLoading ? (
          ...
          <>
            <select
              ...
              disabled={
                sendToAll || (Array.isArray(officers) && officers.length === 0)
              }
            >
              <option value="">
        ...
        {officersError && (
          <div style={{ color: "#c33", fontSize: 13, marginTop: 8 }}>
            {officersError}
          </div>
        )}
      </div>
      ...
      <div className="composer-toolbar">
        <div className="composer-left">
          <button type="button" className="btn btn-secondary" title="Attach">
            📎
          </button>
          <button type="button" className="btn btn-secondary" title="Emoji">
            😊
          </button>
          <button
            type="button"
...
function ComposeWindow({ isOpen, onClose, onSend, stations, replyTo = null }) {
  ...
  return (
    ...
    >
      <div className="compose-window-header" onMouseDown={handleMouseDown}>
          ...
          <button
            ...
            disabled={
              replyTo
                ...
                : !recipientStationId ||
                  (!sendToAll && !recipientOfficerId) ||
                  !formData.body.trim() // Compose mode: need station, recipient, and message
            }
          >
            Send
          </button>
...
export default function Reports() {
  ...
  const myStationId =
    currentUser?.assignedStationId ||
    currentUser?.assigned_station_id ||
    currentUser?.stationInfo?.station_id ||
    null;
    ...
    try {
      ...
      const mapped = conversations.map((c) => {
        ...
        return {
          ...
          name:
            c.otherStationName || c.stationBName || c.stationAName || "Station",
          ...
          sentByMe: last
            ? String(last.sender_station_id) === String(myStationId)
            : false,
      ...
      try {
        ...
        // No fallback: do not add example fire truck driver message
        fireTruckReports = [];
      }
  ...
  useEffect(() => {
    ...
    else if (filterType === "fire-truck")
      filtered = filtered.filter((r) => r.type === "fire-truck-report");
    else if (filterType === "recent")
      filtered = filtered.sort((a, b) => new Date(b.date) - new Date(a.date));
    // 'all' and 'archived' show everything (archived loads separate data)
  ...
  // Reload conversations when switching to/from archived view
  useEffect(() => {
    loadConversations(filterType === "archived");
  }, [filterType === "archived"]);
  ...
  const sendReply = async (payload) => {
    const conversationId =
      replyModal?.to?.conversationId || replyModal?.to?.id || null;
    if (!conversationId) {
    ...
    try {
      ...
      if (messageId && files && files.length > 0) {
        ...
        files.forEach((att) =>
          formData.append("files", att.file ?? att, att.name ?? att.file?.name),
        );
  ...
  useEffect(() => {
    async function loadStations() {
      try {
        ...
        const myStationId =
          me?.assignedStationId ||
          me?.assigned_station_id ||
          me?.stationInfo?.station_id ||
          null;
  ...
  const markAsRead = async (report) => {
    // Only mark incoming (not sent by me) messages that are unread
    if (report.read || report.sentByMe || !report.conversationId) return;
    // Optimistic update — update both states immediately so the badge changes right away
    const applyRead = (prev) =>
      prev.map((r) => (r.id === report.id ? { ...r, read: true } : r));
    setReports(applyRead);
    setFilteredReports(applyRead);
    try {
      await apiClient.patch(`/conversations/${report.conversationId}/read`, {});
    } catch (e) {
      console.error("Failed to mark as read:", e);
      // Revert optimistic update if API call fails
      const revert = (prev) =>
        prev.map((r) => (r.id === report.id ? { ...r, read: false } : r));
      setReports(revert);
      setFilteredReports(revert);
    }
  };
  ...
  const sendNewMessage = async ({
    ...
    try {
      const payload = {
        ...
        recipientOfficerId: recipientOfficerId
          ? Number(recipientOfficerId)
          : null,
      ...
      const messageId = res?.messageId || res?.firstMessageId || null;
  ...
  return (
    <div className="reports-page">
          ...
          <div className="inbox-left-header">
            <div className="inbox-header-content">
              ...
              <div className="search-and-filter">
                ...
                <button
                  ...
                  onClick={() => setComposeModal({ open: true })}
                >
            ...
            <div className="filter-section">
              <div className="filter-buttons">
                ...
                <button
                  className={`filter-btn ${filterType === "fire-truck" ? "active" : ""}`}
                  onClick={() => setFilterType("fire-truck")}
                >
                  <i className="fa-solid fa-truck-fire"></i> Fire Truck
                </button>
                <button
                  className={`filter-btn ${filterType === "spam" ? "active" : ""}`}
          ...
          <div className="inbox-list">
            {filteredReports.length === 0 ? (
              ...
              filteredReports.map((r) => (
                <div
                  key={r.id}
                  className={`inbox-item ${selectedId === r.id ? "active" : ""} ${!r.read && !r.sentByMe ? "unread" : ""} ${r.type === "fire-truck-report" ? "fire-truck-item" : ""}`}
                  onClick={() => {
                    setSelectedId(r.id);
                    markAsRead(r);
                  }}
                ...
                >
                  <div className="item-left">
                    <div className="avatar">
                      <i
                        className={`fa-solid ${r.type === "fire-truck-report" ? "fa-truck-fire" : "fa-file-lines"}`}
                      ></i>
                    </div>
                    {!r.read && !r.sentByMe && (
                  ...
                  <div className="item-body">
                    <div className="item-top">
                      <div
                        className={`item-name${!r.read && !r.sentByMe ? " item-name-unread" : ""}`}
                      >
                        {r.name}
                      </div>
                    ...
                    <div className="item-subject-row">
                      <div
                        className={`item-subject${!r.read && !r.sentByMe ? " item-subject-unread" : ""}`}
                      >
                        {r.subject}
                      </div>
                      {r.sentByMe ? (
                        <span className="read-badge sent-badge">
                          <i className="fa-solid fa-paper-plane"></i> Sent
                        </span>
                      ) : r.read ? (
                        <span className="read-badge read-badge-read">
                          <i className="fa-solid fa-envelope-open"></i> Read
                        </span>
                      ) : (
                        <span className="read-badge unread-badge">
                          <i className="fa-solid fa-envelope"></i> Unread
                        </span>
                      )}
                    </div>
            ...
            <div className="message-card">
              <div className="message-card-header">
                ...
                <div className="message-controls">
                  <button
                    className="gmail-btn archive-btn"
                  ...
                  <button
                    className="gmail-btn delete-btn"
              ...
              <div className="message-sender">
                <div className="sender-avatar">
                  <i
                    className={`fa-solid ${selected.type === "fire-truck-report" ? "fa-truck-fire" : "fa-building"}`}
                  ></i>
                </div>
                <div className="sender-meta">
                  ...
                  {selected.sentByMe && (
                    <div className="message-status-summary">
                      <span className="message-direction-badge sent">
                        You sent the latest message
                      </span>
                    </div>
                  )}
                  {selected.type === "fire-truck-report" &&
                    selected.location && (
                ...
                {(threadMessages && threadMessages.length > 0
                  ...
                  >
                    <div className="thread-message-meta">
                      <span
                        className={`message-direction-badge ${isOutgoingMessage(m) ? "sent" : "received"}`}
                      >
      ...
      {confirmState.open && (
        <ConfirmModal
          title={
            confirmState.type === "delete" ? "Delete Report" : "Archive Report"
          }
          message={

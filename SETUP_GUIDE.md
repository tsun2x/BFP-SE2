...
### 8d. Set up the `.env` file

Create a `.env` file in the `mobile-firetruck-expo/` folder with:

```env
EXPO_PUBLIC_BASE_URL=https://YOUR-TUNNEL-URL.trycloudflare.com
EXPO_PUBLIC_SUPABASE_URL=https://ccmwlhbrwrmqjxkgjswj.supabase.co
EXPO_PUBLIC_SUPABASE_ANON_KEY=<same anon key as backend - ask Mark>
```

> ⚠️ Without this `.env` file, the app will crash on startup with a `[runtime not ready]` Supabase error.
...
2. **📍 Tracking:**
   ...
   - Admins can see where you are on a live map
...
### `firetrucks` table — Registered firetrucks
| Column | What It Is |
|--------|----------|
| `truck_id` | Unique truck number (e.g., 1) |
| `truck_code` | Short code (e.g., `FT-001`) |
| `assigned_station_id` | Which station this truck belongs to |
| `is_active` | Whether the truck is active |

> ⚠️ **You MUST add at least one row here** before firetruck tracking will work. The `firetruck_location_history` table has a foreign key to `firetrucks.truck_id`, so if the truck doesn't exist here, GPS locations can't be saved and the truck won't appear on the admin map.
...
### `firetruck_location_history` table — GPS breadcrumb archive
| Column | What It Is |
|--------|----------|
| `truck_id` | Which truck (FK to `firetrucks.truck_id`) |
| `latitude`, `longitude` | GPS coordinates |
| `recorded_at` | Timestamp of the reading |

The admin map polls this table every 5 seconds to show firetruck icons.
...
EXPO_PUBLIC_BASE_URL=https://YOUR-TUNNEL-URL.trycloudflare.com
EXPO_PUBLIC_SUPABASE_URL=https://ccmwlhbrwrmqjxkgjswj.supabase.co
EXPO_PUBLIC_SUPABASE_ANON_KEY=<same anon key as backend>
```
...
|------|----------------|
| [CHANGES_MARCH_10_2026.md](CHANGES_MARCH_10_2026.md) | March 10 2026 session — map icons, firetruck tracking fixes, notification dedup |

          ...
          <button 
            ...
            disabled={
              replyTo 
                ...
                : !recipientStationId || (!sendToAll && !recipientOfficerId) || !formData.body.trim()  
            }
          >
            Send
          </button>
...
export default function ReportSub() {
  ...
  useEffect(() => {
    if (selectedId) loadThread(selectedId); else setThreadMessages([]);
  }, [selectedId]);
  ...
  const performConfirm = () => {
    ...
    if (confirmState.type === 'delete') {
      apiClient.delete(`/conversations/${id}`).then(()=>loadConversations()).catch(()=>loadConversations());
    }
  ...
  const sendReply = async (payload) => {
    ...
    if (!conversationId) { setReplyModal({ open:false, to:null }); return; }
    try {
    ...
    } catch (e) { setReplyModal({ open:false, to:null }); setToastType('error'); setToastMessage('Failed to send reply.'); }
  };
  ...
  useEffect(() => {
    async function loadStations() {
      try {
        ...
        const filtered = myStationId ? all.filter((s) => String(s.station_id) !== String(myStationId)) : all;
  ...
  const sendNewMessage = async ({ recipientStationId, recipientOfficerId, sendToAll, subject, body, attachments: files }) => {
    try {
      const payload = {
    ...
    } catch (e) { console.error('Failed to send message:', e); setToastType('error'); setToastMessage('Failed to send message.'); }
  };

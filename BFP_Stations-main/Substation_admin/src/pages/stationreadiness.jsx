...
export default function StationReadiness() {
  ...
  const calculateReadiness = () => {
    const criticalFail = !checklist.firetruck || !checklist.radio || !checklist.driver || !checklist.scba;
  ...
  const addChecklistItem = () => {
    ...
    const key = newItemName.toLowerCase().replace(/\s+/g, '_');
    
    // Add to checklist and labels
    setChecklist(prev => ({ ...prev, [key]: false }));
    setChecklistLabels(prev => ({ ...prev, [key]: newItemName }));
  ...
  const updateChecklistItem = () => {
    if (!editingItem || !editItemName.trim()) {
      setToastMessage("Item name cannot be empty");
      setToastType("error");
      return;
    }

    setChecklistLabels(prev => ({ ...prev, [editingItem]: editItemName }));
    setEditingItem(null);
    setEditItemName("");
  ...
  const confirmDelete = () => {
    ...
    // Remove from both checklist and labels
    setChecklist(prev => {
  ...
  // ===== FILTERED ITEMS =====
  const equipmentItems = Object.entries(checklistLabels)
    .filter(([key]) => ['firetruck', 'scba', 'hoses', 'radio', 'water'].includes(key));

  const personnelItems = Object.entries(checklistLabels)
      ...
      <div className="readiness-container">
        ...
        <div className="readiness-content">
          ...
          <div className="status-section">
            ...
            <button className="confirm-button" onClick={() => setModalOpen(true)} disabled={loading}>

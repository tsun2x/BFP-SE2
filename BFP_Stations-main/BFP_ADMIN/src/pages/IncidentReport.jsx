...
export default function IncidentReport() {
  ...
  const applyPreviewToForm = (preview) => {
    ...
    setFormData((prev) => ({
      ...
      phoneNumber: userEditedPhone
        ? prev.phoneNumber
        : preview.phoneNumber || preview.number || "",
      location: preview.location || "",
      incidentType: preview.incidentType || "",
      narrative: preview.narrative || "",
      alarmLevel: normalizeAlarmLevel(preview.alarmLevel) || "",
    }));
  ...
  const loadIncidentPreviewByAlarmId = async (alarmId) => {
    ...
    try {
      const data = await apiClient.get(
        `/incidents/${numericAlarmId}/report-data`,
      );
      ...
      applyPreviewToForm({
        ...
        alarmLevel:
          alarm.current_alarm_level ||
          alarm.initial_alarm_level ||
          report.alarm_level ||
          "",
        coordinates: {
      ...
      console.warn(
        "[IncidentReport] Failed to load report data by alarm id:",
        error,
      );
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      const routeAlarmId =
        routePreview?.alarmId || routePreview?.id || location.state?.alarmId;
      ...
      const storedAlarmId = Number(
        parsedStoredPreview?.alarmId || parsedStoredPreview?.id || 0,
      );
      ...
      loadIncidentPreviewByAlarmId(
        preview.alarmId || preview.id || location.state?.alarmId,
      );
    } catch (error) {
      console.warn(
        "[IncidentReport] Failed to load notification preview:",
        error,
      );
    }
  ...
  useEffect(() => {
    const handleIncidentStatusUpdated = (event) => {
      const eventAlarmId =
        Number(event?.detail?.alarmId || event?.detail?.id || 0) || null;
      const routePreview = location.state?.incidentPreview || null;
      const activeAlarmId =
        Number(
  ...
  const normalizeAlarmLevel = (level) => {
    ...
    if (
      /\d+(st|nd|rd|th) Alarm/.test(level) ||
      /st Alarm|nd Alarm|rd Alarm|th Alarm/.test(level)
    )
      return level;
  ...
  const submitIncidentReport = async () => {
    ...
    try {
      ...
      if (!token && import.meta.env.DEV) {
        ...
        setToastMessage(
          `(DEV) Incident created successfully! Alarm ID: ${fakeAlarmId}`,
        );
      ...
      setToastMessage(
        `Incident created successfully! Alarm ID: ${response.alarmId}`,
      );
      setToastType("success");
            ...
            <div className="voice-console-body">
              {PHP_BACKEND_URL ? (
                ...
                <div style={{ padding: "1rem" }}>
                  Set `VITE_PHP_BACKEND_URL` to enable the legacy station voice
                  console.
                </div>
              )}
            </div>

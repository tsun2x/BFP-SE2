  ...
  try {
    ...
    try {
      ...
      const { data: stations, error: stErr } = await supabase
        .from("fire_stations")
        .select(
          "station_id, station_name, latitude, longitude, contact_number",
        );
      ...
      if (stations && stations.length > 0) {
        ...
        (readinessRows || []).forEach((r) => {
          if (!readinessMap.has(r.station_id))
            readinessMap.set(r.station_id, r.status);
        });
        ...
        const allEligible = stations
          .filter((s) => {
            ...
            if (!pass)
              console.log(
                `[KNN] Station ${s.station_id} (${s.station_name}) FILTERED OUT — readiness: ${rs}`,
              );
            return pass;
          })
          .map((s) => ({
        ...
        const onlineEligible = allEligible.filter((s) =>
          onlineIds.has(s.station_id),
        );
        ...
        if (scored.length > 0) {
          ...
          console.log(
            "[KNN] No online substations. Dispatching directly to MAIN ADMIN.",
          );
  ...
  async (req, res) => {
    try {
      ...
      if (io) {
        ...
        console.log(
          `[AcceptAPI] Emitted call-accepted to alarm-${alarmId} (station ${stationId})`,
        );
      }
  ...
  async (req, res) => {
    try {
      ...
      const locationFallback =
        location ||
        (alarm.user_latitude && alarm.user_longitude
          ? `${alarm.user_latitude}, ${alarm.user_longitude}`
          : "Unknown Location");

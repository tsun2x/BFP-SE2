...
export default function useTwilioVoice(identity: string | null, authToken: string | null) {
  ...
  const fetchToken = useCallback(async (): Promise<string | null> => {
    ...
    const isJwtLike = /^[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+$/.test(safeAuthToken);
    if (!isJwtLike) {
      setError('Invalid session token. Please log out and log back in.');
      return null;
    }
    ...
    try {
      const res = await fetch(`${API_URL}/api/twilio/token`, {
        ...
        headers: {
          ...
          Authorization: `Bearer ${safeAuthToken}`,

...
function AppContent() {
  ...
  useEffect(() => {
    ...
    // Firetruck status/alarm update from driver (deduplicated — notify only on real changes)
    const lastTruckStatus = {};
    socket.on('truck-status-update', (data) => {
      try {
        console.log('[Frontend] Received truck-status-update:', data);
        const tid = data.truckId || 'unknown';
        const key = `${data.fireStatus}|${data.alarmLevel}`;
        if (lastTruckStatus[tid] === key) return; // same status — skip notification
        lastTruckStatus[tid] = key;
              ...
              <div className="modal-card" style={{ textAlign: 'center', maxWidth: '420px', padding: '32px 28px', borderRadius: '16px', animation: 'pulse 1.5s infinite' }}>
                ...
                <div style={{ marginTop: '20px', display: 'flex', gap: '12px', justifyContent: 'center' }}>
                  <button onClick={() => {
                    ...
                    acceptCall(call.id);
                    if (twilioIncomingCall) { try { twilioAcceptIncoming(); } catch (e) {} } else { setPendingTwilioAccept(true); }
                    navigate('/incident-report');

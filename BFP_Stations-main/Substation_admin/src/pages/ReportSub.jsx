  ...
  return (
    <div className="composer-form">
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        {officersError && (
          <div style={{ color: '#c33', fontSize: 13, marginTop: 8 }}>{officersError}</div>
        )}
      </div>
      ...
      <div className="composer-toolbar">
        <div className="composer-left">
          ...
          <button type="button" className="btn btn-secondary" title="Formatting">
            A
          </button>
        </div>
        <div className="composer-right">
          ...
          <button
            ...
            onClick={() =>
              onSend({ recipientStationId, recipientOfficerId, sendToAll, subject, body })
            }
  ...
  return (
    ...
    >
      <div className="compose-window-header" onMouseDown={handleMouseDown}>
        <div className="compose-window-title">{replyTo ? 'Reply to Message' : 'New Message'}</div>
        <div className="compose-window-controls">
          ...
          />
          <button className="compose-window-control close" onClick={onClose} title="Close" />
        </div>
      </div>
      ...
      <div className="compose-window-body">
        ...
        <div className="compose-actions">
          ...
          <button
            ...
            disabled={
              replyTo
                ? !formData.body.trim() || (sendToAll && !recipientOfficerId)
                : !recipientStationId ||
...
export default function ReportSub() {
  ...
  const loadConversations = async (archived = false) => {
    try {
      const endpoint = archived ? '/conversations?archived=true' : '/conversations';
  ...
  useEffect(() => {
    ...
    if (composeModal.open) loadStations();
  }, [composeModal.open]);
  ...
  const markAsRead = async (report) => {
    ...
    const applyRead = (prev) => prev.map((r) => (r.id === report.id ? { ...r, read: true } : r));
    ...
    try {
      ...
      const revert = (prev) => prev.map((r) => (r.id === report.id ? { ...r, read: false } : r));
      setReports(revert);
  ...
  const sendNewMessage = async ({
    ...
    try {
      ...
      if (messageId && files && files.length > 0) {
        const formData = new FormData();
        files.forEach((att) => formData.append('files', att.file, att.name));
        const token = localStorage.getItem('authToken');
        await fetch(`${API_BASE}/messages/${messageId}/attachments`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },

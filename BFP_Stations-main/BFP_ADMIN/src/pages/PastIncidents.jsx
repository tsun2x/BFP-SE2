...
function stationLabel(incident) {
  return (
    ...
    (incident.assigned_station_id
      ? `Station #${incident.assigned_station_id}`
      : "Unassigned")
  );
}
...
export default function PastIncidents() {
  ...
  const stationOptions = useMemo(() => {
    ...
    for (const incident of allIncidents) {
      const label = stationLabel(incident);
      if (!seen.has(label)) {
        seen.add(label);
        stations.push(label);
      }
  ...
  const scopedIncidents = useMemo(() => {
    ...
    return allIncidents.filter((incident) => {
      ...
      if (stationFilter !== "all" && stationLabel(incident) !== stationFilter) {
        return false;
      }
  ...
  }, [allIncidents, selectedRange, stationFilter, statusFilter, query]);

  const stationBreakdown = useMemo(() => {
    const map = new Map();
    for (const incident of scopedIncidents) {
      const key = stationLabel(incident);
      const existing = map.get(key) || { incidents: 0, dispatchLagTotal: 0, dispatchLagCount: 0 };
      existing.incidents += 1;

      const lag = computeDispatchLagMinutes(incident);
      if (lag !== null) {
        existing.dispatchLagTotal += lag;
        existing.dispatchLagCount += 1;
      }
    ...
    return [...map.entries()]
      .map(([station, row]) => ({
        ...
        share: scopedIncidents.length
          ? (row.incidents / scopedIncidents.length) * 100
          : 0,
        avgDispatchLag:
          row.dispatchLagCount > 0
            ? row.dispatchLagTotal / row.dispatchLagCount
            : null,
      }))
  ...
  const statusSummary = useMemo(() => {
    ...
    for (const incident of scopedIncidents) {
      ...
      else if (["Dispatched", "On Scene", "Under Control"].includes(status))
        statusMap.active += 1;
  ...
  return (
    <div className="past-incidents-page">
      <div className="past-incidents-header">
        ...
        <p>Historical station analytics with quick operational drilldown.</p>
        <button
      ...
      <div className="past-filters">
        <input
          ...
          placeholder="Search caller, phone, location, station, alarm id..."
        />
      ...
      <div className="past-grid">
        ...
        <section className="past-card">
          <h3>By Fire Station</h3>
          {loading ? (
            <p className="past-empty">Loading stations…</p>
          ) : stationBreakdown.length === 0 ? (
            <p className="past-empty">No station data for this filter.</p>
          ) : (
                ...
                <tbody>
                  {stationBreakdown.map((row) => (
                    <tr key={row.station}>
                      ...
                      <td>{formatDurationMinutes(row.avgDispatchLag)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

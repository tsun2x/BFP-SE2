...
export default function IncidentReportModal({ alarmId, onClose }) {
  ...
  useEffect(() => {
    ...
    apiCall(`/incidents/${alarmId}/report-data`)
      .then((data) => {
        setReportData(data);
        // Pre-fill form from existing report or alarm data
        if (data.report) {
          setForm({
          ...
          // Pre-fill narrative from alarm_response_log details if available
  ...
  return (
    <div style={styles.backdrop} onClick={onClose}>
      <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
        ...
        {!loading && error && !reportData && <div style={styles.error}>{error}</div>}
        ...
        {!loading && reportData && (
          <>
            ...
            {/* Download buttons — always visible, use latest data */}

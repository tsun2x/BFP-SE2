...
export function downloadPDF({ alarm, timeline, report }) {
  ...
  autoTable(doc, {
    ...
    columnStyles: { 0: { fontStyle: 'bold', cellWidth: 50 } },
    body: [
      ['Incident Type', fmt(report?.incident_type || alarm?.incident_type)],
      ['Initial Alarm Level', fmt(alarm?.initial_alarm_level)],
      ['Final Alarm Level', fmt(alarm?.current_alarm_level)],
      ['Coordinates', alarm?.user_latitude ? `${alarm.user_latitude}, ${alarm.user_longitude}` : 'N/A'],
      ['Narrative', fmt(report?.narrative)],
    ],
    margin: { left: 14, right: 14 },
  });
  y = doc.lastAutoTable.finalY + 6;
  ...
  const doc = new Document({
    sections: [{
      children: [
        ...
        infoTable([
          ['Incident Type', report?.incident_type || alarm?.incident_type],

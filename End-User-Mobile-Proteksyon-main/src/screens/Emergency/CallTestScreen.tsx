...
export const CallTestScreen = ({ navigation, route }: any) => {
  ...
  const [status, setStatus] = useState<string>(
    "Ready to start an emergency call.",
  );
  ...
  const createOfferAndSend = async () => {
    ...
    try {
      ...
      if (locationStatus !== "granted") {
        ...
        setStatus(
          "Location permission denied. Turn on location services to call the nearest station.",
        );
        setCallPhase("error");
        Alert.alert(
          "Location Required",
      ...
      appendLog(
        `Got GPS location lat=${latitude}, lon=${longitude}. Creating alarm via KNN...`,
      );
      setStatus("Finding the nearest fire station...");

      const phoneNumber = "9997778888";
      const alarmResponse = await fetch(
        `${NODE_API_URL}/api/enduser/create-alarm`,
        {
          method: "POST",
      ...
      if (!alarmResponse.ok) {
        ...
        Alert.alert(
          "Emergency Error",
          alarmData.message || "Failed to create alarm.",
        );
        return;
      }
      ...
      setCallPhase("inCall");
      setStatus(
        "Alarm created. Station will contact you. Voice call support (Twilio) coming soon.",
      );
    } catch (err: any) {
      try {
      ...
      setStatus(
        "We could not complete the emergency call. Please try again, or use the hotline numbers on the previous screen.",
      );
      setCallPhase("error");
    } finally {
      setCreating(false);

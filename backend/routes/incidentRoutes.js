      ...
      if (stations && stations.length > 0) {
        ...
        const allEligible = stations
          .filter((s) => {
            ...
            const pass = !rs || rs !== 'NOT_READY';
            if (!pass) console.log(`[KNN] Station ${s.station_id} (${s.station_name}) FILTERED OUT — readiness: ${rs}`);
            return pass;
          })
          .map((s) => ({
            ...s,
        ...
        if (scored.length === 0 && allEligible.length > 0) {
          console.log(`[KNN] Eligible but offline: ${allEligible.map(s => `${s.station_id}(${s.station_name})`).join(', ')}`);
        }
        ...
        if (scored.length > 0) {
          ...
          const io = req.app.get('io');
          if (io) {
            // NOTE: Do NOT emit failover-redirect here — civilian socket hasn't
            // joined alarm-${alarmId} yet. The HTTP response already tells the
            // client to call ADM_MAIN directly.
...
router.get('/incidents', authenticateToken, requireRoles(['admin', 'substation_admin', 'driver']), async (req, res) => {
  try {
    ...
    let query = supabase
      .from('alarms')
      .select(
        `alarm_id,end_user_id,user_latitude,user_longitude,initial_alarm_level,current_alarm_level,status,assigned_station_id,call_time,dispatch_time,resolve_time,users!end_user_id(full_name,phone_number),fire_stations!assigned_station_id(station_name),alarm_response_log!alarm_id(details)`
      )
...
router.get('/incidents/:alarmId/report-data', authenticateToken, requireRoles(['admin', 'substation_admin']), async (req, res) => {
  try {
    ...
    const { data: alarm, error: alarmErr } = await supabase
      .from('alarms')
      .select('alarm_id,end_user_id,user_latitude,user_longitude,initial_alarm_level,current_alarm_level,status,assigned_station_id,assigned_truck_id,call_time,dispatch_time,resolve_time,users!end_user_id(full_name,phone_number),fire_stations!assigned_station_id(station_name,address,contact_number)')
      .eq('alarm_id', alarmId)
      .single();
...
router.post('/incidents/:alarmId/submit-report', authenticateToken, requireRoles(['admin', 'substation_admin']), async (req, res) => {
  try {
    ...
    const locationFallback = location ||
      (alarm.user_latitude && alarm.user_longitude ? `${alarm.user_latitude}, ${alarm.user_longitude}` : 'Unknown Location');
